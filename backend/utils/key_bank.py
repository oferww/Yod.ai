...
class KeyBank:
    ...
    - COHERE_API_KEY: comma/semicolon/space-separated list
    - or numbered: COHERE_API_KEY_1 .. COHERE_API_KEY_20
    Env:
    ...
    def from_env(cls) -> "KeyBank":
        ...
        # 1) Multi-value COHERE_API_KEY
        raw = os.getenv("COHERE_API_KEY", "").strip()
        if raw:
            # split by common separators
            parts = [p.strip() for p in raw.replace(";", ",").replace(" ", ",").split(",")]
            keys.extend([p for p in parts if p])

        # 2) Numbered keys: COHERE_API_KEY_1..20
        if not keys:
            for i in range(1, 21):
                v = os.getenv(f"COHERE_API_KEY_{i}")
                if v and v.strip():
                    keys.append(v.strip())

        if not keys:
            raise EnvironmentError(
                "No Cohere keys found. Set COHERE_API_KEY (comma-separated) or COHERE_API_KEY_1..N"
            )
